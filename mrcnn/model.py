...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            model = KM.Model([input_image, input_image_meta, input_anchors],
                             ...
                             name='mask_rcnn')
            plot_model(model, to_file='mode_MaskRCNN.png')
            plot_model(model, to_file='model_MaskRCNN_more detail.png', show_shapes=True)  # more detail
    ...
    def unmold_detections_superpixels(self, detections, mrcnn_mask, original_image_shape,
                          image_shape, window, images_original):
        """Reformats the detections of one image from the format of the neural
        network output to a format suitable for use in the rest of the
        application.
        ...
        for i in range(N):
            # Convert neural network mask to full size mask
            full_mask = utils.unmold_mask_superpixels(masks[i], boxes[i], original_image_shape, images_original)
            full_masks.append(full_mask)
        full_masks = np.stack(full_masks, axis=-1)\
            if full_masks else np.empty(masks.shape[1:3] + (0,))
    ...
    def detect_superpixels(self, images, images_original, verbose=0):
        """Runs the detection pipeline.
        ...
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks =\
                self.unmold_detections_superpixels(detections[i], mrcnn_mask[i],
                                       image.shape, molded_images[i].shape,
                                       windows[i], images_original)
            results.append({
                "rois": final_rois,
                "class_ids": final_class_ids,
                "scores": final_scores,
